...
void new (class User {
    constructor() {
        ...
        this.us_prod_id = document.querySelector('[name="us_prod_id"]')
    ...
    eventHandler = () => {
        document.querySelectorAll(".editInfo")
            ...
            document.querySelectorAll(".deleteConfirm").forEach(async (el) => el.addEventListener("click", this.getUserID));
            
    };

    getUserID = async ({ target }) => {
        try {
            $('#kt_modal_1').modal('show')
            console.log(`us_prod_id:${target.dataset.user}`);
            this.us_prod_id.value=target.dataset.user;
